...
export const PlayerScreen = {
  ...
  async selectNextEpisodeStreamByAutoPlayPolicy(
    ...
    const bingeGroupOnlyManualMode =
      shouldAutoSelectInManualMode && !settings.autoplayNextEpisode && preferBingeGroup;
      ...
      if (status === "confirmed") {
        ...
        if (this.startupAudioGateActive) {
          ...
          const preferredTargets =
            typeof this.getStartupAudioPreferredTargets === "function"
              ? this.getStartupAudioPreferredTargets()
              : [];
          const matchesStartupPreference =
  ...
  async playStreamByUrl(
    ...
    const prioritizeWebOsRemoteMkvPlayback =
      Environment.isWebOS() && !nextEngineFsState && this.isCurrentSourceLikelyMkv(streamUrl);
  ...
  renderBitmapSubtitleAtCurrentTime({ force = false } = {}) {
    ...
    const verticalOffsetPx =
      normalizeSubtitleVerticalOffset(style.verticalOffset) * -0.02 * viewportHeight;
  ...
  getSubtitleLanguageRailItems() {
    ...
    const preferredKeys = new Set(
      preferredTargets.map((target) => normalizeSubtitleLanguageKey(target))
    );
          ...
          !this.sourcesLoading && !filtered.length
            ...
            : filtered
                .map((stream, index) => {
                  ...
                  const addonLogoUrl = getPlayerSourceLogoDisplayUrl(stream.addonLogo, () =>
                    this.scheduleSourceLogoRender()
                  );
                  return `
